...
class FishKiosk(tk.Frame):
    def __init__(self, master):
        ...
        self.root = master
        self.root.title("Pastil Kiosk - Fish Menu")
        ...
        tk.Button(self, text="← Back", font=("Helvetica", 12, "bold"), width=8,
         ...
         # --- Fish Menu ---
        tk.Label(self, text="🍗 Fish Menu", font=("Helvetica", 32, "bold"), **self.label_style).pack(pady=10)
        self.fish_frame = tk.Frame(self, bg="#282c73")
        self.fish_frame.pack(pady=20)

        columns = 4
        for index, (key, (name, price)) in enumerate(self.Fish.items()):
            row = index // columns
            col = index % columns
            self.create_menu_item(key, name, price, parent=self.fish_frame, row=row, col=col, image=self.images.get(key))
    ...
    def add_to_cart(self, item_id):
        if item_id in self.Fish:
            item_name, item_price = self.Fish[item_id]
        else:
    ...
    def edit_cart_item(self, index, window):
        ...
        # Look for the price in Fish or AddOns
        if any(item_name == name for name, _ in self.Fish.values()):
            item_price = next(price for name, price in self.Fish.values() if name == item_name)
        else:
